...
dll_insert_named :: proc "contextless" (first, last: ^^$T, after, node: ^T, $next, $prev: string, nil_value: ^T = nil) {
...
dll_insert_default :: proc "contextless" (first, last: ^^$T, after, node: ^T, nil_value: ^T = nil) {
    dll_insert_named(first, last, after, node, "next", "prev", nil_value)
}

dll_insert :: proc{dll_insert_default, dll_insert_named}

dll_push_back_named :: proc "contextless" (first, last: ^^$T, node: ^T, $next, $prev: string, nil_value: ^T = nil) {
    dll_insert_named(first, last, last^, node, next, prev, nil_value)
}

dll_push_back_default :: proc "contextless" (first, last: ^^$T, node: ^T, nil_value: ^T = nil) {
    dll_push_back_named(first, last, node, "next", "prev", nil_value)
}

dll_push_back :: proc{dll_push_back_default, dll_push_back_named}

dll_push_front_named :: proc "contextless" (first, last: ^^$T, node: ^T, $next, $prev: string, nil_value: ^T = nil) {
    dll_insert_named(first, last, nil_value, node, next, prev, nil_value)
}

dll_push_front_default :: proc "contextless" (first, last: ^^$T, node: ^T, nil_value: ^T = nil) {
    dll_push_front_named(first, last, node, "next", "prev", nil_value)
}

dll_push_front :: proc{dll_push_front_default, dll_push_front_named}

dll_remove_named :: proc "contextless" (first, last: ^^$T, node: ^T, $next, $prev: string, nil_value: ^T = nil) {
    node_next := field_ptr(node, next)
    node_prev := field_ptr(node, prev)
...
dll_remove_default :: proc "contextless" (first, last: ^^$T, node: ^T, nil_value: ^T = nil) {
    dll_remove_named(first, last, node, "next", "prev", nil_value)
}

dll_remove :: proc{dll_remove_default, dll_remove_named}

sll_queue_push_named :: proc "contextless" (first, last: ^^$T, node: ^T, $next: string, nil_value: ^T = nil) {
    node_next := field_ptr(node, next)
    if first^ == nil_value {
        first^ = node
        last^ = node
...
sll_queue_push_default :: proc "contextless" (first, last: ^^$T, node: ^T, nil_value: ^T = nil) {
    sll_queue_push_named(first, last, node, "next", nil_value)
}

sll_queue_push :: proc{sll_queue_push_default, sll_queue_push_named}

sll_queue_push_front_named :: proc "contextless" (first, last: ^^$T, node: ^T, $next: string, nil_value: ^T = nil) {
    node_next := field_ptr(node, next)
    if first^ == nil_value {
        first^ = node
        last^ = node
...
sll_queue_push_front_default :: proc "contextless" (first, last: ^^$T, node: ^T, nil_value: ^T = nil) {
...
sll_queue_pop_named :: proc "contextless" (first, last: ^^$T, $next: string, nil_value: ^T = nil) {
    if first^ == last^ {
        first^ = nil_value
        last^ = nil_value
    } else {
...
sll_queue_pop_default :: proc "contextless"(first, last: ^^$T, nil_value: ^T = nil) {
    sll_queue_pop_named(first, last, "next", nil_value)
}

sll_queue_pop :: proc{sll_queue_pop_default, sll_queue_pop_named}

sll_stack_push_named :: proc "contextless" (first: ^^$T, node: ^T, $next: string) {
    field_ptr(node, next)^ = first^
    first^ = node
}

sll_stack_push_default :: proc "contextless" (first: ^^$T, node: ^T) {
    sll_stack_push_named(first, node, "next")
}

sll_stack_push :: proc{sll_stack_push_default, sll_stack_push_named}

sll_stack_pop_named :: proc "contextless" (first: ^^$T, $next: string) {
    first^ = field_ptr(first^, next)^
}

sll_stack_pop_default :: proc "contextless" (first: ^^$T) {
